...
def use_xiaolou_models_only() -> bool:
    value = os.getenv("JAAZ_XIAOLOU_MODELS_ONLY", "true").strip().lower()
...
async def get_models() -> list[ModelInfo]:
    ...
    for provider in config.keys():
        if provider in ['ollama']:
            continue
        if xiaolou_only and provider not in ['xiaolou-vertex', 'xiaolou-dashscope']:
            continue
...
async def list_tools() -> list[ToolInfoJson]:
    ...
    for tool_id, tool_info in tool_service.tools.items():
        ...
        if xiaolou_only and provider != 'xiaolou':
            continue
        provider_config = config.get(provider, {})
        provider_api_key = provider_config.get('api_key', '').strip()
        if provider not in ['comfyui', 'xiaolou'] and not provider_api_key:
            continue
        res.append({
            'id': tool_id,
