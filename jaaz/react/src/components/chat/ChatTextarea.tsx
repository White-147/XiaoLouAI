...
import { getSafeDefaultTools, ToolInfo } from '@/api/model'
...
type ChatMode = 'agent' | 'image' | 'video'

const CHAT_MODE_STORAGE_KEY = 'jaaz_chat_mode'
...
const getInitialChatMode = (): ChatMode => {
  ...
  return storedMode === 'agent' || storedMode === 'image' || storedMode === 'video'
    ? storedMode
    : 'agent'
}

const persistSelectedTools = (allTools: ToolInfo[], selectedTools: ToolInfo[]) => {
  const selectedToolIds = new Set(selectedTools.map((tool) => tool.id))
  window.localStorage.setItem(
    'disabled_tool_ids',
    JSON.stringify(
      allTools.filter((tool) => !selectedToolIds.has(tool.id)).map((tool) => tool.id)
    )
  )
}
...
const ChatTextarea: React.FC<ChatTextareaProps> = ({
  ...
  const { textModel, selectedTools, setSelectedTools, allTools, setShowLoginDialog } = useConfigs()
  ...
  const hasAppliedStoredModeRef = useRef(false)
  ...
  const getToolsForMode = useCallback(
    (mode: ChatMode) => {
      if (mode === 'agent') {
        return getSafeDefaultTools(allTools)
      }

      const typedTools = allTools.filter((tool) => tool.type === mode)
      if (typedTools.length === 0) {
        return []
      }

      if (allTools.every((tool) => tool.provider === 'xiaolou')) {
        const preferredTool = getSafeDefaultTools(allTools).find(
          (tool) => tool.type === mode
        )
        return preferredTool ? [preferredTool] : [typedTools[0]]
      }
  ...
  const applyChatMode = useCallback(
    (mode: ChatMode, showWarning = true) => {
      setChatMode(mode)
      ...
      setSelectedTools(modeTools)
      persistSelectedTools(allTools, modeTools)

      if (showWarning && modeTools.length === 0) {
        toast.warning(
          mode === 'image'
    ...
    [allTools, getToolsForMode, setSelectedTools]
  )

  useEffect(() => {
    if (hasAppliedStoredModeRef.current || allTools.length === 0) {
      return
    }

    hasAppliedStoredModeRef.current = true
    applyChatMode(chatMode, false)
  }, [allTools.length, applyChatMode, chatMode])
          ...
          >
            {images.map((image) => (
              ...
              >
                <img
                  key={image.file_id}
                  src={normalizeJaazApiUrl(`/api/file/${image.file_id}`)}
                  alt="Uploaded image"
            ...
            {CHAT_MODES.map((mode) => {
              ...
              return (
                <Button
                  ...
                  className={cn(
                    'h-8 gap-1 rounded-sm px-2 text-xs font-medium',
                    isActive
                      ? 'bg-primary text-primary-foreground shadow-sm hover:bg-primary/90 hover:text-primary-foreground'
                      : 'text-muted-foreground hover:text-foreground'
                  )}
                  onClick={() => applyChatMode(mode.id)}
                >
