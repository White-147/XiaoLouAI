...
export default function TopMenu({
  ...
  const isEmbeddedInXiaolou = isXiaolouEmbedded()

  if (isEmbeddedInXiaolou) return null
    ...
    >
      <div className="flex items-center gap-8">
        ...
        {!isEmbeddedInXiaolou ? (
          <Button
            variant={
              window.location.pathname === '/assets' ? 'default' : 'ghost'
            }
            size="sm"
      ...
      <div className="flex items-center gap-2">
        ...
        <LanguageSwitcher />
        {!isEmbeddedInXiaolou ? <ThemeButton /> : null}
        {!isEmbeddedInXiaolou ? <UserMenu /> : null}
      </div>
    </motion.div>
  )
}
