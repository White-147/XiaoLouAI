...
G13-post-6 required-gate-ratchet: active only for the signed remote advisory evidence scope. Added non-required GitHub Actions workflow `.github/workflows/synthetic-e2e-advisory.yml` with check context `Synthetic browser E2E advisory`; it runs `npm --prefix .\XIAOLOU-main run test:e2e:synthetic` on windows-latest/Node 22/Chrome, uploads no artifacts, and is not part of branch protection. Remote run evidence is pending until the workflow is pushed and completes. `Build and static gates` remains the only required check.
```
