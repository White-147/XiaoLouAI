...
```text
Status: advisory/non-blocking gates only; synthetic browser E2E remains non-required, with a first local runtime baseline plus G13-post-5d completion at 5/5 effective green rounds. G13-post-6 has limited execution signoff to add remote non-required synthetic E2E advisory context/evidence while keeping `Build and static gates` as the only required check.
...
G13-post-5 synthetic-e2e-smoke-harness: foundation plus interaction flows added a non-required Playwright synthetic browser smoke script using synthetic auth/localStorage, intercepted synthetic Control API fixtures, fake storage/job mocks, production preview on port 3100, and no real material. It now covers static route smoke, API-center client navigation, Playground synthetic requests, email login, personal registration, image create submit/job polling, asset fake PUT upload, and toolbox synthetic route/job polling. First local runtime baseline on May 5, 2026: 3 consecutive `test:e2e:synthetic` runs passed 13/13, Playwright reported 29.7-30.0s, and outer PowerShell timing was 31.07-31.88s. Flake policy: keep retries=0 and workers=1, investigate any failure or any single run above 60s, do not count an immediate rerun as a green baseline, and keep CI required E2E / required gate promotion deferred until repeated advisory greens and owner signoff.
...
G13-post-6-preflight required-gate-ratchet-plan-owner-signoff-record: done. Required-check scope remains the existing `Build and static gates` check only; synthetic E2E should continue advisory/non-required for now. Future ratchet signoff must name the exact check context, evidence set, branch-protection before/after state, rollback owner, and baseline-reset conditions. Rollback for any future ratchet is to restore branch protection to the single `Build and static gates` required check, remove any newly required advisory check from the required list, rerun CI/protection readback, and update README plus handoffs. Execution signoff status: not granted.
G13-post-6 required-gate-ratchet: done for the signed remote advisory evidence scope. Added non-required GitHub Actions workflow `.github/workflows/synthetic-e2e-advisory.yml` with check context `Synthetic browser E2E advisory`; it runs `npm --prefix .\XIAOLOU-main run test:e2e:synthetic` on windows-latest/Node 22/Chrome, uploads no artifacts, and is not part of branch protection. First remote advisory evidence: run 25381961070 on commit 2e7c553 succeeded, job `Synthetic browser E2E advisory` succeeded, and the E2E log reported 13 passed in 55.7s. Required CI run 25381961063 also succeeded on the same commit. `Build and static gates` remains the only required check.
```
