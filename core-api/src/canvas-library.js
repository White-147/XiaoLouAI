...
const DEFAULT_CANVAS_IMAGE_MODEL_ID = "gemini-3-pro-image-preview";

// Single-point compat layer for old workflow files imported from external sources.
// These are NOT selectable models — they only exist to migrate stale workflow JSON
// without breaking the save/load flow.
// gpt-image-1.5 entry: legacy alias for externally-imported old workflows only.
//   It MUST NOT appear in any UI list, capability map, or new workflow data.
const LEGACY_CANVAS_IMAGE_MODEL_ALIASES = new Map([
  ["gpt-image-1.5", DEFAULT_CANVAS_IMAGE_MODEL_ID],   // old workflow import compat — backend only
  ["gemini-pro", DEFAULT_CANVAS_IMAGE_MODEL_ID],
  ["imagen-3.0-generate-002", DEFAULT_CANVAS_IMAGE_MODEL_ID],
]);

function normalizeCanvasWorkflowImageModelId(value) {
  const normalized = typeof value === "string" ? value.trim() : "";
  if (!normalized) return normalized;
  return LEGACY_CANVAS_IMAGE_MODEL_ALIASES.get(normalized) || normalized;
}
...
function sanitizeWorkflowNode(node) {
  ...
  if (typeof clean.imageModel === "string") {
    clean.imageModel = normalizeCanvasWorkflowImageModelId(clean.imageModel);
  }

  if (typeof clean.model === "string") {
    clean.model = normalizeCanvasWorkflowImageModelId(clean.model);
  }
...
function sanitizeWorkflowNodes(nodes) {
  ...
  return nodes.map(sanitizeWorkflowNode);
}
...
function buildCanvasLibraryRoutes(/* store */) {
  ...
  return [
    ...
    route("GET", "/api/canvas/workflows/:id", ({ params, req, url }) => {
      ...
      const wf = sanitizeWorkflow(JSON.parse(fs.readFileSync(filePath, "utf8")));
    ...
    route("GET", "/api/canvas/public-workflows/:id", ({ params }) => {
      ...
      return raw(sanitizeWorkflow(JSON.parse(fs.readFileSync(filePath, "utf8"))));
    }),
    ...
    routeWithStatus("POST", "/api/canvas/assets/:type", 200, async ({ params, req }) => {
      ...
      // Historically this endpoint returned `/library/${type}/${filename}` but
      // only `/canvas-library/*` is proxied by Vite and served by
      // serveCanvasLibrary(). Returning the served prefix directly ensures
      // freshly uploaded canvas assets render without a 404 after round-trip.
      return raw({ success: true, id, filename, url: `/canvas-library/${type}/${filename}` });
    }),
      ...
      for (const file of files) {
        if (file.endsWith(".json")) {
          try {
            ...
            meta.url = `/canvas-library/${type}/${meta.filename}`;
            assets.push(meta);
          } catch { /* skip */ }
        }
      }
