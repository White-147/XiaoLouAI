...
模块运行。它现在只作为迁移期 / 只读兼容面；生产控制面工作应落在
`../control-plane-dotnet`。

## 提供的能力

- 项目、脚本、资产、故事板、视频、配音、任务、钱包、企业、工具箱和画布相关的
  legacy route 实现与迁移参考。
...
本节仅作为 legacy/local 对照说明。生产 Windows-native hosting 应直接服务
`XIAOLOU-main/dist`，并只按根 README 与 `deploy/windows/*` 示例中的显式 allowlist
反代 `.NET` Control API。

如需本地兼容对照，可以用一个域名同时暴露前端和 legacy compatibility API：
...
以下仅作为本地 legacy compatibility 探针。生产环境中等价的 project/canvas/create
surface 应由 `.NET` Control API 提供。
