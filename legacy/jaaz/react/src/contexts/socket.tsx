...
function getSocketServerUrl() {
  const configured = String(import.meta.env.VITE_JAAZ_SOCKET_URL || '').trim()
  if (configured) return configured

  const embeddedApiBase = String(import.meta.env.VITE_JAAZ_API_BASE_PATH || '').trim()
  if (embeddedApiBase) return window.location.origin

  return import.meta.env.DEV ? 'http://localhost:57988' : window.location.origin
}
