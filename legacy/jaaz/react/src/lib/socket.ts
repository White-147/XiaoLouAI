...
import { fileNameFromJaazUrl, postXiaolouAgentAsset } from './xiaolou-embed'
    ...
    switch (type) {
      ...
      case ISocket.SessionEventType.ImageGenerated:
        postXiaolouAgentAsset({
          fileUrl: data.image_url || String(data.file?.dataURL || ''),
          fileName: fileNameFromJaazUrl(
            data.image_url || String(data.file?.dataURL || '')
          ),
          name: fileNameFromJaazUrl(
            data.image_url || String(data.file?.dataURL || '')
          ),
          mediaKind: 'image',
      ...
      case ISocket.SessionEventType.VideoGenerated:
        postXiaolouAgentAsset({
          fileUrl: data.video_url || String(data.file?.dataURL || ''),
          fileName: fileNameFromJaazUrl(
            data.video_url || String(data.file?.dataURL || '')
          ),
          name: fileNameFromJaazUrl(
            data.video_url || String(data.file?.dataURL || '')
          ),
