...
G13-post-6 required-gate-ratchet: active，但仅限已签收的远端 advisory evidence 范围。新增非 required GitHub Actions workflow `.github/workflows/synthetic-e2e-advisory.yml`，check context 为 `Synthetic browser E2E advisory`；它在 windows-latest/Node 22/Chrome 上运行 `npm --prefix .\XIAOLOU-main run test:e2e:synthetic`，不上传 artifact，也不进入 branch protection。远端运行证据待该 workflow push 后完成。`Build and static gates` 仍是唯一 required check。
```
