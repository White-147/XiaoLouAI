...
```text
Status: 当前只保留 advisory/non-blocking gates；synthetic browser E2E 仍是 non-required，已记录首轮本地 runtime baseline，G13-post-5d 已完成 5/5 effective green rounds。G13-post-6 已获得有限执行签收：只补远端 non-required synthetic E2E advisory context/evidence，并保持 `Build and static gates` 为唯一 required check。
...
G13-post-6-preflight required-gate-ratchet-plan-owner-signoff-record: done。Required-check scope 仍只保留现有 `Build and static gates`；synthetic E2E 当前继续保持 advisory/non-required。未来 ratchet 签收必须写明精确 check context、evidence set、branch-protection before/after、rollback owner，以及 baseline-reset 条件。任何未来 ratchet 的回滚方案是把 branch protection 恢复为只要求 `Build and static gates`，从 required 列表移除新增 advisory check，重新读取 CI/protection 状态，并同步 README 与 handoff。执行签收状态：未授权。
G13-post-6 required-gate-ratchet: done，但仅限已签收的远端 advisory evidence 范围。新增非 required GitHub Actions workflow `.github/workflows/synthetic-e2e-advisory.yml`，check context 为 `Synthetic browser E2E advisory`；它在 windows-latest/Node 22/Chrome 上运行 `npm --prefix .\XIAOLOU-main run test:e2e:synthetic`，不上传 artifact，也不进入 branch protection。首轮远端 advisory evidence：commit 2e7c553 上的 run 25381961070 success，job `Synthetic browser E2E advisory` success，E2E log 报告 13 passed，runtime 55.7s；同一 commit 的 required CI run 25381961063 也 success。`Build and static gates` 仍是唯一 required check。
```
