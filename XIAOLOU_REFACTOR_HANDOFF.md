...
G13-post-6 required-gate-ratchet: done (limited to non-required remote advisory check context/evidence; `Build and static gates` remains the only required check)
```
...
DONE G13-post-6 required-gate-ratchet (limited signed scope complete: non-required remote advisory check context/evidence added; `Build and static gates` remains sole required check)
```

### 下一棒提示词

```text
当前没有无条件 READY NEXT owner。G13-post-6 limited required-gate-ratchet 已完成：只新增 non-required remote `Synthetic browser E2E advisory` check context/evidence，并保持 `Build and static gates` 为唯一 required check。先读取三份 handoff、README Deferred CI/Test Gate Follow-Up、当前 dirty worktree、G13-post-5a/5b/5c/5d/5d-completion/G13-post-6-preflight/G13-post-6 records、XIAOLOU-main test:e2e:synthetic/Playwright harness、backend/frontend route/type static scan records、legacy dependency gate、当前 CI/branch-protection 状态。

当前 required-check scope 仍只能是现有 `Build and static gates`；`Synthetic browser E2E advisory` 是远端 non-required advisory context，不在 branch protection required list。任何下一棒若没有新的显式 owner signoff，只能继续 advisory evidence、监控/记录或等待签收，不得新增/修改 required check、workflow 或 branch protection。

若后续 owner 明确签收并要求把 `Synthetic browser E2E advisory` 或其他 advisory gate 提升为 required，必须在执行前再次确认：精确 check context、CI workflow/check-run 来源、branch-protection before/after、rollback owner、是否需要更多远端 advisory 绿灯、以及是否发生 harness/fixture/Vite config/auth/session/create/upload/toolbox/polling 变更导致 baseline 重置。保持 DTO、route path、status code、response shape、auth/permission/account-scope 行为、frontend exported API names、polling/transport/DB owner 不变；不删除 api.ts compatibility wrappers 或 legacy verifier/deploy evidence；不读取或上传真实 auth/provider/payment/storage/operator material、production dump/snapshot、真实 DB fixture 或真实 object storage。
```
