...
if (-not $SkipRollbackSnapshot) {
  ...
  foreach ($item in @(
    ...
    [ordered]@{ path = "backend\services\model-runtime\local-model-worker-sidecar"; exclude = @() }
  )) {
...
$paths = @(
  ...
  "$RuntimeRoot\docs",
  "$RuntimeRoot\backend\services\model-runtime\local-model-worker-sidecar",
...
Copy-Item -Path "$SourceRoot\backend\services\model-runtime\local-model-worker-sidecar\*" -Destination "$RuntimeRoot\backend\services\model-runtime\local-model-worker-sidecar" -Recurse -Force
...
$sharedCacheRoot = Resolve-EnvValue -Text $envText -Name "XIAOLOU_SHARED_CACHE_ROOT" -DefaultValue "D:\soft\cache"
$sharedProgramRoot = Resolve-EnvValue -Text $envText -Name "XIAOLOU_SHARED_PROGRAM_ROOT" -DefaultValue "D:\soft\program"
New-Item -ItemType Directory -Force -Path $sharedCacheRoot, "$sharedCacheRoot\pip", "$sharedCacheRoot\cuda\compute-cache", "$sharedCacheRoot\xiaolou-video-replace-weights", $sharedProgramRoot, "$sharedProgramRoot\dotnet-userhome", "$sharedProgramRoot\dotnet-userhome\tools", "$sharedProgramRoot\nodejs\node_global", "$sharedProgramRoot\Python\UserBase", "$sharedProgramRoot\Python\UserBase\Scripts", "$sharedProgramRoot\ms-playwright" | Out-Null
...
$postgresMinimumPoolSize = Resolve-EnvValue -Text $envText -Name "Postgres__MinimumPoolSize" -DefaultValue "0"
$postgresMaximumPoolSize = Resolve-EnvValue -Text $envText -Name "Postgres__MaximumPoolSize" -DefaultValue "60"
$postgresTimeoutSeconds = Resolve-EnvValue -Text $envText -Name "Postgres__TimeoutSeconds" -DefaultValue "15"
...
$envValues = [ordered]@{
  ...
  XDG_CACHE_HOME = "$sharedCacheRoot\tooling-cache"
  TMP = $tempRoot
  TEMP = $tempRoot
  DOTNET_CLI_HOME = "$sharedProgramRoot\dotnet-userhome"
  DOTNET_BUNDLE_EXTRACT_BASE_DIR = "$sharedCacheRoot\dotnet-bundle"
  NUGET_PACKAGES = "$sharedCacheRoot\nuget\packages"
  NUGET_HTTP_CACHE_PATH = "$sharedCacheRoot\nuget\v3-cache"
  NUGET_PLUGINS_CACHE_PATH = "$sharedCacheRoot\nuget\plugins-cache"
  NUGET_SCRATCH = "$tempRoot\NuGetScratch"
  NPM_CONFIG_CACHE = "$sharedCacheRoot\npm"
  NPM_CONFIG_PREFIX = "$sharedProgramRoot\nodejs\node_global"
  PIP_CACHE_DIR = "$sharedCacheRoot\pip"
  PIP_CONFIG_FILE = "$sharedCacheRoot\pip\pip.ini"
  PYTHONPYCACHEPREFIX = "$sharedCacheRoot\python-pycache"
  PYTHONUSERBASE = "$sharedProgramRoot\Python\UserBase"
  UV_CACHE_DIR = "$sharedCacheRoot\uv"
  POETRY_CACHE_DIR = "$sharedCacheRoot\poetry"
  PIPENV_CACHE_DIR = "$sharedCacheRoot\pipenv"
  PLAYWRIGHT_BROWSERS_PATH = "$sharedProgramRoot\ms-playwright"
  MAVEN_USER_HOME = "$sharedCacheRoot\maven\.m2"
  GRADLE_USER_HOME = "$sharedCacheRoot\gradle-user-home"
  COURSIER_CACHE = "$sharedCacheRoot\coursier-cache"
  SBT_OPTS = "-Dsbt.boot.directory=$sharedCacheRoot\scala\sbt-boot -Dsbt.global.base=$sharedCacheRoot\scala\sbt-global -Dsbt.ivy.home=$sharedCacheRoot\scala\ivy2"
  HF_HOME = "$sharedCacheRoot\huggingface"
  HF_HUB_CACHE = "$sharedCacheRoot\huggingface\hub"
  HUGGINGFACE_HUB_CACHE = "$sharedCacheRoot\huggingface\hub"
  TRANSFORMERS_CACHE = "$sharedCacheRoot\huggingface\transformers"
  TORCH_HOME = "$sharedCacheRoot\torch"
  MODELSCOPE_CACHE = "$sharedCacheRoot\modelscope"
  CUDA_CACHE_PATH = "$sharedCacheRoot\cuda\compute-cache"
  VR_WEIGHTS_ROOT = "$sharedCacheRoot\xiaolou-video-replace-weights"
