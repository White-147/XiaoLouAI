...
$postgresMinimumPoolSize = if ($env:Postgres__MinimumPoolSize) { $env:Postgres__MinimumPoolSize } else { "0" }
$postgresMaximumPoolSize = if ($env:Postgres__MaximumPoolSize) { $env:Postgres__MaximumPoolSize } else { "60" }
$postgresTimeoutSeconds = if ($env:Postgres__TimeoutSeconds) { $env:Postgres__TimeoutSeconds } else { "15" }
$postgresCommandTimeoutSeconds = if ($env:Postgres__CommandTimeoutSeconds) { $env:Postgres__CommandTimeoutSeconds } else { "30" }
$postgresKeepAliveSeconds = if ($env:Postgres__KeepAliveSeconds) { $env:Postgres__KeepAliveSeconds } else { "30" }
