...
export const TikTokPostModal: React.FC<TikTokPostModalProps> = ({
    ...
    const checkAuthStatus = async (session: string) => {
        try {
            const response = await fetch(buildCanvasApiUrl(`/tiktok-post/status?sessionId=${session}`));
    ...
    const handleLogin = async () => {
        ...
        try {
            const response = await fetch(buildCanvasApiUrl('/tiktok-post/auth'));
            const data = await response.json();
    ...
    const handleLogout = async () => {
        if (sessionId) {
            try {
                await fetch(buildCanvasApiUrl('/tiktok-post/logout'), {
                    method: 'POST',
    ...
    const handlePost = async () => {
        ...
        try {
            const response = await fetch(buildCanvasApiUrl('/tiktok-post/post'), {
                method: 'POST',
                headers: {
    ...
    const fullMediaUrl = resolveCanvasMediaUrl(mediaUrl);
                        ...
                        <div className="flex flex-col items-center gap-4 py-8">
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-sm text-neutral-400 mt-1">
                                    Sign in to post videos directly from the canvas
                                </p>
                            </div>
                            <button
                                onClick={handleLogin}
