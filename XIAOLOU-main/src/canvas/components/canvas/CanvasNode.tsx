          ...
          >
            ...
            >
              {data.resultUrl ? (
                ...
                <div className="flex min-h-[340px] flex-col items-center justify-center gap-3 px-6 text-center">
    ...
    >
      ...
      <div className="relative group/nodecard">
        ...
        {data.type === NodeType.IMAGE && isSuccess && data.resultUrl && (
          ...
          >
            <div className="flex items-center gap-1 px-2 py-1.5 bg-[#171612]/95 rounded-full border border-[rgba(245,244,239,0.12)] shadow-xl backdrop-blur-md">
              {/* Multi-angle and Upload buttons - Hidden for storyboard-generated scenes */}
        ...
        <div
          className={`relative ${data.type === NodeType.VIDEO ? 'w-[385px]' : 'w-[365px]'} rounded-2xl border transition-all duration-300 flex flex-col shadow-2xl ${isDark ? 'bg-[#1d1b16]' : 'bg-[#f9f8f6]'} ${selected ? 'border-blue-500/50 ring-1 ring-blue-500/30' : isDark ? 'border-[rgba(245,244,239,0.12)]' : 'border-[rgba(26,26,25,0.12)]'}`}
        >
          ...
          {!shouldHideFloatingLabel && (
            isEditingTitle ? (
              ...
              onPointerDown={(e) => e.stopPropagation()}
                className="absolute top-2 select-text text-sm px-2 py-0.5 rounded font-medium bg-blue-500/20 text-blue-200 outline-none border border-blue-400 whitespace-nowrap"
              style={{ right: 'calc(100% + 8px)', minWidth: '60px' }}
            />
          ) : (
            <div
              className="absolute top-2 select-none text-sm px-2 py-0.5 rounded font-medium transition-colors cursor-text whitespace-nowrap text-neutral-600"
        ...
        {shouldShowControls && data.type !== NodeType.TEXT && !(data.prompt && data.prompt.startsWith('Extract panel #')) && (
          (data.type === NodeType.VIDEO || data.type === NodeType.LOCAL_VIDEO_MODEL) ? (
            <div className="absolute top-[calc(100%+12px)] left-1/2 -translate-x-1/2 w-[520px] flex justify-center z-[100]">
              <VideoSettingsPanel
                data={data}
                inputUrl={inputUrl}
                isLoading={isLoading}
