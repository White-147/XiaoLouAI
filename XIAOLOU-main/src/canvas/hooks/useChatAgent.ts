...
export function useChatAgent(): UseChatAgentReturn {
    ...
    const refreshSessions = useCallback(async () => {
        ...
        try {
            const response = await fetch(buildCanvasApiUrl('/chat/sessions'));
            if (response.ok) {
    ...
    const loadSession = useCallback(async (targetSessionId: string) => {
        ...
        try {
            const response = await fetch(buildCanvasApiUrl(`/chat/sessions/${targetSessionId}`));
            if (!response.ok) {
    ...
    const deleteSession = useCallback(async (targetSessionId: string) => {
        try {
            await fetch(buildCanvasApiUrl(`/chat/sessions/${targetSessionId}`), {
                method: 'DELETE',
            });
    ...
    const sendMessage = useCallback(async (
        ...
        try {
            const response = await fetch(buildCanvasApiUrl('/chat'), {
                method: 'POST',
