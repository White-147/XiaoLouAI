...
import { buildCanvasApiUrl, toCanvasBackendLibraryPath } from '../integrations/twitcanvaRuntimePaths';
...
export const useVideoEditor = ({ nodes, updateNode }: UseVideoEditorOptions) => {
    ...
    const handleExportTrimmedVideo = useCallback(async (
        ...
        try {
            ...
            const response = await fetch(buildCanvasApiUrl('/trim-video'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    videoUrl: toCanvasBackendLibraryPath(videoUrl),
                    startTime: trimStart,
                    endTime: trimEnd,
                    nodeId
                })
