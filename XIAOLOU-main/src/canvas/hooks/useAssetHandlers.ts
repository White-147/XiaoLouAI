...
import { DEFAULT_XIAOLOU_TEXT_TO_IMAGE_MODEL_ID, getDefaultCanvasImageResolution } from '../config/canvasImageModels';
import { DEFAULT_XIAOLOU_IMAGE_TO_VIDEO_MODEL_ID } from '../config/canvasVideoModels';
import {
    buildCanvasApiUrl,
    resolveCanvasMediaUrl,
    toCanvasBackendLibraryPath,
...
export const useAssetHandlers = ({
    ...
    const handleSelectAsset = useCallback((
        ...
        const createNode = (resultAspectRatio?: string, aspectRatio?: string) => {
            ...
            const newNode: NodeData = {
                ...
                resultUrl: url,
                resultAspectRatio,
                model: isVideo ? DEFAULT_XIAOLOU_IMAGE_TO_VIDEO_MODEL_ID : DEFAULT_XIAOLOU_TEXT_TO_IMAGE_MODEL_ID,
                videoModel: isVideo ? DEFAULT_XIAOLOU_IMAGE_TO_VIDEO_MODEL_ID : undefined,
                imageModel: !isVideo ? DEFAULT_XIAOLOU_TEXT_TO_IMAGE_MODEL_ID : undefined,
                aspectRatio: aspectRatio || '16:9',
                resolution: isVideo ? 'Auto' : getDefaultCanvasImageResolution(DEFAULT_XIAOLOU_TEXT_TO_IMAGE_MODEL_ID)
            };
    ...
    const handleSaveAssetToLibrary = useCallback(async (name: string, category: string) => {
        ...
        try {
            const mediaKind = nodeToSnapshot.type === NodeType.VIDEO ? 'video' : 'image';
            const model = mediaKind === 'video'
                ? (nodeToSnapshot.videoModel || nodeToSnapshot.model)
                : (nodeToSnapshot.imageModel || nodeToSnapshot.model);

            if (canUseXiaolouAssetBridge()) {
                await createXiaolouAsset({
                    name,
                    category,
                    sourceUrl: nodeToSnapshot.resultUrl,
                    mediaKind,
                    prompt: nodeToSnapshot.prompt,
                    model,
                    aspectRatio: nodeToSnapshot.aspectRatio,
                    resultAspectRatio: nodeToSnapshot.resultAspectRatio
                });
                return;
            }
            ...
            const response = await fetch(buildCanvasApiUrl('/library'), {
                ...
                body: JSON.stringify({
                    sourceUrl: toCanvasBackendLibraryPath(nodeToSnapshot.resultUrl),
                    name: name,
                    category: category
                })
            });
    ...
    const handleContextUpload = useCallback((file: File) => {
        ...
        reader.onload = async (e) => {
            ...
            try {
                ...
                const response = await fetch(buildCanvasApiUrl(`/assets/${type}`), {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        data: base64Data,
                ...
                const resultUrl = resolveCanvasMediaUrl(responseData.url);
