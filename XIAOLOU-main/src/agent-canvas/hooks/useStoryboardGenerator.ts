...
export const useStoryboardGenerator = ({ onCreateNodes, viewport }: UseStoryboardGeneratorProps) => {
    ...
    const generateScripts = useCallback(async () => {
        ...
        try {
            const response = await fetch(buildCanvasApiUrl('/storyboard/generate-scripts'), {
                method: 'POST',
    ...
    const brainstormStory = useCallback(async () => {
        ...
        try {
            const response = await fetch(buildCanvasApiUrl('/storyboard/brainstorm-story'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    characterDescriptions: state.selectedCharacters.map(c => ({
    ...
    const optimizeStory = useCallback(async () => {
        ...
        try {
            const response = await fetch(buildCanvasApiUrl('/storyboard/optimize-story'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    story: state.story,
    ...
    const generateComposite = useCallback(async () => {
        ...
        try {
            const response = await fetch(buildCanvasApiUrl('/storyboard/generate-composite'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    const createStoryboardNodes = useCallback(() => {
        ...
        const newNodes: Partial<NodeData>[] = state.scripts.map((script, index) => {
            ...
            return {
                ...
                // Default to XiaoLou's primary image creation model
                model: DEFAULT_XIAOLOU_TEXT_TO_IMAGE_MODEL_ID,
                imageModel: DEFAULT_XIAOLOU_TEXT_TO_IMAGE_MODEL_ID,
                aspectRatio: '16:9',
                resolution: getDefaultCanvasImageResolution(DEFAULT_XIAOLOU_TEXT_TO_IMAGE_MODEL_ID),
