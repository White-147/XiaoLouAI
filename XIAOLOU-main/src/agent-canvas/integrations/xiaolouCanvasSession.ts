...
export function getXiaolouCanvasDraftStorageKey(scope?: string | null) {
  const actorScope = getXiaolouActorId() || DEFAULT_DRAFT_SCOPE;
  const normalizedScope = typeof scope === 'string' ? scope.trim() : '';
  return normalizedScope
    ? `xiaolou:agent-canvas:draft:${actorScope}:${normalizedScope}`
    : `xiaolou:agent-canvas:draft:${actorScope}`;
}
