...
import { getNodeWidth as getUserNodeWidth, getNodeContentHeight } from '../../utils/nodeGeometry';
...
const getNodeWidth = (node: NodeData, allNodes?: NodeData[]): number => {
    // Honour user-resized media nodes first. The shared helper ignores the
    // override for non-resizable types and falls back to the legacy default,
    // so existing layouts (Editors, Camera Angle, ...) render unchanged.
    if (node.type === NodeType.IMAGE ||
        node.type === NodeType.VIDEO ||
        node.type === NodeType.LOCAL_IMAGE_MODEL ||
        node.type === NodeType.LOCAL_VIDEO_MODEL) {
        return getUserNodeWidth(node);
    }
    ...
    // Remaining: TEXT / AUDIO / STORYBOARD / CAMERA_ANGLE — historically
    // rendered at 365px.
    return 365;
};
...
const getNodeHeight = (node: NodeData, allNodes?: NodeData[]): number => {
    // For resizable media nodes, defer to the shared helper so bounding
    // boxes track the user's corner-drag resizes without drifting.
    if (node.type === NodeType.IMAGE ||
        node.type === NodeType.VIDEO ||
        node.type === NodeType.LOCAL_IMAGE_MODEL ||
        node.type === NodeType.LOCAL_VIDEO_MODEL) {
        ...
        >
            ...
            {isGrouped && (
                ...
                >
                    ...
                    {group.storyContext && onEditStoryboard && (
                        <button
                            onClick={(e) => {
                                e.stopPropagation();
                                onEditStoryboard(group.id);
                            }}
