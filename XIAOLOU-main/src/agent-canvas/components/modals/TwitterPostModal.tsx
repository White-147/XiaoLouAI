...
export const TwitterPostModal: React.FC<TwitterPostModalProps> = ({
    ...
    const checkAuthStatus = async (session: string) => {
        try {
            const response = await fetch(buildCanvasApiUrl(`/twitter/status?sessionId=${session}`));
    ...
    const handleLogin = async () => {
        ...
        try {
            const response = await fetch(buildCanvasApiUrl('/twitter/auth'));
            const data = await response.json();
    ...
    const handleLogout = async () => {
        if (sessionId) {
            try {
                await fetch(buildCanvasApiUrl('/twitter/logout'), {
                    method: 'POST',
    ...
    const handlePost = async (skipMedia = false) => {
        ...
        try {
            ...
            const response = await fetch(buildCanvasApiUrl('/twitter/post'), {
                method: 'POST',
                headers: {
                        ...
                        <div className="flex flex-col items-center gap-4 py-8">
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-sm text-neutral-400 mt-1">
                                    Sign in to post directly from the canvas
                                </p>
                            </div>
                            <button
                                onClick={handleLogin}
