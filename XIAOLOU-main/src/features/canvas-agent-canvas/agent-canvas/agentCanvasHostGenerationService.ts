...
type AgentCanvasGenerationServices = Pick<
  CanvasHostServices,
  | "generateImage"
  | "generateVideo"
  | "recoverGeneration"
  ...
  while (Date.now() < deadline) {
    ...
    if (shouldUseProjectAssetFallback(projectId)) {
      try {
        ...
        console.warn("[AgentCanvasCreate] waitForCreateVideoResult optional listAssets failure:", err);
      }
    }
...
export function createAgentCanvasHostGenerationService(
  resolveReadyProjectId: () => Promise<string>,
): AgentCanvasGenerationServices {
  return {
