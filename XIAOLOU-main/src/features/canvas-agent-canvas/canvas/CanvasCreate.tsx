...
import { createCanvasHostGenerationService } from "./canvasHostGenerationService";
import { createCanvasHostAssetService } from "./canvasHostAssetService";
import { createCanvasHostProjectService } from "./canvasHostProjectService";
import { createCanvasHostSaveService } from "./canvasHostSaveService";
import { useCanvasHostProjectLoad } from "./canvasHostProjectLoad";
...
export default function CanvasCreate() {
  ...
  // ── Save-state refs ────────────────────────────────────────────────────────
  // URL project IDs win over localStorage so existing projects never save to a
  // stale session project before the async load completes.
  ...
  const services = useMemo((): CanvasHostServices => ({
    ...
    initialTheme: theme,
      ...
      ...createCanvasHostGenerationService(resolveReadyProjectId),

      // ── Assets ──────────────────────────────────────────────────────────────
      ...createCanvasHostAssetService(resolveReadyProjectId),

      // ── Canvas projects ─────────────────────────────────────────────────────
      ...createCanvasHostProjectService({
        actorIdRef,
        canvasProjectIdRef,
        canvasProjectUpdatedAtRef,
        canvasProjectBaseTitleRef,
